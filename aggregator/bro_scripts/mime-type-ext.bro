...
export {
  redef enum Notice::Type += {
          ...
          Download_Sensitive_Type,
          Download_Sensitive_Extension,
  };
  ...
  const suspicious_types = 
    ...
    /text\/x-lua/             |
    /text\/x-php/             |
    /application\/x-dosexec/ &redef;

  const suspicious_extensions =
    /((ba|tc|c|z|fa|ae|k)?sh)$/                       |
    /([pP][eE][rR][lL]|[aA][lL]|[pP][lL]|[pP][mM])$/  |
    /([rR][bB])$/                                     |
    /([pP][yY]|[wW][sS][gG][iI])$/                    |
    /([aA][wW][kK])$/                                 |
    /([tT][cC][lL]|[tT][kK])$/                        |
...
event file_state_remove(f: fa_file)
{
  ...
  {
    message = fmt("'%s' downloaded as '%s'", mime_ext, f$info$filename);
    note = fmt("Mime_Type_Ext_Mismatch");
    ...
    Log::write(Mime::LOG, [$ts=f$info$ts, $fuid=f$info$fuid, 
                          ...
                          $source=f$info$source, $mime_type=mime_ext, 
                          $filename=f$info$filename, 
                          $note=note, $msg=message]);

    NOTICE([$note=Mime_Type_Ext_Mismatch,
      $msg=fmt("%s downloaded as %s", mime_ext, f$info$filename),
      $id=id]);
...
event file_state_remove(f: fa_file)
{
  if(f$is_orig) return;
  if(!f?$info) return;
  if(f$info?$filename && f$info$filename == "f.txt") return;
  ...
  if (f?$info && f$info?$mime_type)
  {
	  #local mime_ext = meta$mime_type;
	  local mime_ext = f$info$mime_type;
    if(suspicious_types in mime_ext)
    {
      message = fmt("Suspicious Mime type download Mime_type: '%s'", mime_ext);
      note = fmt("Download_Sensitive_Type");
      ...
      filename = f$info?$filename ? f$info$filename : "-";
      Log::write(Mime::LOG, [$ts=f$info$ts, $fuid=f$info$fuid, 
                            $tx_hosts=f$info$tx_hosts, 
                            $rx_hosts=f$info$rx_hosts, 
                            $conn_uids=f$info$conn_uids, 
      ...
      NOTICE([$note=Download_Sensitive_Type,
        $msg=fmt("Suspicious Mime type download Mime_type: '%s'", mime_ext),
        $id=id]);
      return;
    }
  }
  if (f$info?$filename)
  {
    ...
    if(/\./ in f$info$filename)
    {
      if(suspicious_extensions in file_ext)
      {
        message = fmt("Suspicious file extension download Filename: '%s'", f$info$filename);
        note = fmt("Download_Sensitive_Extension");
        ...
        NOTICE([$note=Download_Sensitive_Extension,
          $msg=fmt("Suspicious file extension download Mime_type: '%s' Filename: '%s'", mime_ext, f$info$filename),
          $id=id]);
      }
